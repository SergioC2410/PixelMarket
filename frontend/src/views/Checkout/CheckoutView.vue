  ...
  <div class="cart-container">
    ...
    <div class="item-list">
      <div v-for="item in cartItems" :key="item.id" class="cart-item">
        ...
        <div class="item-details">
          ...
          <p>
            Precio en Pesos: {{ formatCurrency(item.price * item.quantity) }}<br />
            Precio en Dólares: {{ formatCurrencyUSD(item.price * item.quantity) }}<br />
            Descuento: {{ item.discountPercentage }}%<br />
            Precio con Descuento en Pesos: {{ formatCurrency(calculatePriceWithDiscount(item)) }}<br />
    ...
    <div class="checkout-summary">
      ...
      <div class="summary-row">
        <span>Subtotal:</span>
        <span>Pesos: {{ formatCurrency(subtotal) }} | Dólares: {{ formatCurrencyUSD(subtotal) }}</span>
      </div>
      <div class="summary-row">
        <span>Impuestos (16%):</span>
        <span>Pesos: {{ formatCurrency(taxes) }} | Dólares: {{ formatCurrencyUSD(taxes) }}</span>
      </div>
      <div class="summary-row">
        <span>Envío:</span>
        <span>Pesos: {{ formatCurrency(shippingCost) }} | Dólares: {{ formatCurrencyUSD(shippingCost) }}</span>
      </div>
      <div class="summary-row">
        <span>Descuento Total:</span>
        <span>Pesos: {{ formatCurrency(totalDiscount) }} | Dólares: {{ formatCurrencyUSD(totalDiscount) }}</span>
      </div>
      <div class="summary-row total">
        <span>Total:</span>
        <span>Pesos: {{ formatCurrency(totalWithDiscount) }} | Dólares: {{ formatCurrencyUSD(totalWithDiscount) }}</span>
      </div>
